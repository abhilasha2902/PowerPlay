...
export default function ReserveWidget({ pricePerNight, nights, rating, reviewCount }: ReserveWidgetProps) {
  ...
  return (
    <>
      ...
      <div className="reserve-mobile-bar" aria-label="Reservation">
        <div className="reserve-mobile-bar-info">
          <span className="reserve-mobile-bar-price">₹{pricePerNight.toLocaleString('en-IN')}</span>
          <span className="reserve-mobile-bar-unit">night</span>
        </div>
        <button type="button" className="reserve-button reserve-mobile-bar-button">Reserve</button>
      </div>
    </>
  )
}
